...
import json
import moviepy as mpy # may have changes
...
class ModelWrapper(LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.test_cfg.compute_scores:
            ...
            psnr_tensor = compute_psnr(rgb_gt, rgb_pred)
            all_metrics = {
        ...
        if self.test_cfg.save_image:
            psnr_dict = {}  # 用于存储PSNR值
            indexs = batch["target"]["index"][0]
            for index, (color, psnr_item) in zip(indexs, zip(output.color[0], psnr_tensor)):
                psnr_dict[f"image_{index:0>6}"] = psnr_item.item()
                save_image(color, path / scene / f"color/{index:0>6}.png")

            psnr_json_path = path / scene / "psnr_values.json"
            with open(psnr_json_path, 'w') as json_file:
                json.dump(psnr_dict, json_file, indent=4)
            # print("Saved PSNR per image:", psnr_dict)
        ...
        if self.test_cfg.save_video:
            ...
            save_video(
                ...
                path / scene / f"{scene}_frame_{frame_str}.mp4",
            )
        ...
        if self.test_cfg.save_compare:
            ...
            save_image(comparison, path / scene / "compare_gt_pred.png")
