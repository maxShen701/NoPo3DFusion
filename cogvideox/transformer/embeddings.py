...
class CogVideoXPatchEmbed(nn.Module):
    ...
    def forward(self, text_embeds: torch.Tensor, image_embeds: torch.Tensor):
        ...
        image_embeds = self.pre_proj(image_embeds)
...
class MemoryPatchEmbed(nn.Module):
    def __init__(
        self,
        patch_size: int = 2,
        in_channels: int = 16,
    ...
    def _get_positional_embeddings(self, sample_height, sample_width, sample_frames):
        post_patch_height = sample_height // self.patch_size
        post_patch_width = sample_width // self.patch_size
        post_time_compression_frames = (sample_frames - 1) // 4 + 1  # 默认 temporal_compression_ratio=4
    ...
    def forward(self, text_embeds: torch.Tensor, latent_image: torch.Tensor):
        """
        Args:
            text_embeds: (B, text_len, text_embed_dim)
            latent_image: (B, T, C=16, H, W)
        Returns:
            embeds: (B, text_len + patch_tokens, embed_dim)
        """
        text_embeds = self.text_proj(text_embeds)

        B, T, C, H, W = latent_image.shape
        x = latent_image.view(-1, C, H, W)  # (B*T, C, H, W)
        x = self.proj(x)
        x = x.view(B, T, self.embed_dim, x.shape[2], x.shape[3])
        x = x.flatten(3).transpose(2, 3)  # (B, T, HW, C)
        x = x.flatten(1, 2)  # (B, T*HW, C)

        embeds = torch.cat([text_embeds, x], dim=1)

        if self.use_positional_embeddings or self.use_learned_positional_embeddings:
            embeds = embeds + self.pos_embedding.to(embeds.device, dtype=embeds.dtype)
